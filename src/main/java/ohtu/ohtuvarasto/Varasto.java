...
public class Varasto {
    ...
    public void lisaaVarastoon(double maara) {
        if (maara < 0) {
            return;       // virhetilanteessa voidaan tehdä tällainen pikapoistuminenkin!
        }
        if (maara <= paljonkoMahtuu()) {
            saldo = saldo + maara;          // omia aksessoreita voi kutsua ihan suoraan sellaisinaan
        } else {
    ...
    public double otaVarastosta(double maara) {
        if (maara < 0) {
            return 0.0;   // virhetilanteessa voidaan tehdä tällainen pikapoistuminenkin!
        }
